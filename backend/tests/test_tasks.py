...
async def test_tasks_user_fixture(session: AsyncSession):
    """Create a test user for task operations."""
    user = User(
        username=f"taskuser_{uuid4()}",
...
def test_task_operations(client: TestClient, test_tasks_user: tuple[User, str]):
    ...
    assert list_data["tasks"][0]["task_id"] == task_id

    # 4. Update the task (PATCH)
    update_response = client.patch(
        f"/api/users/{user_id}/tasks/{task_id}",
        headers=auth_headers,
        json={"title": "Updated Task Title", "is_complete": True},
    )
    assert update_response.status_code == 200
    updated_data = update_response.json()
    assert updated_data["title"] == "Updated Task Title"
    assert updated_data["is_complete"] is True
    ...
    assert get_updated_data["is_complete"] is True
